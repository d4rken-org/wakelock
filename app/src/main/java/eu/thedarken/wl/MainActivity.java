...
public class MainActivity extends Activity {
	...
	private void setLevel(int level) {
		if (level == 7) {
			int cnt = 0;
			if (!isVisible(level0))
				fadeIn(level0, 150 * cnt++);
			if (!isVisible(level1))
				fadeIn(level1, 150 * cnt++);
			if (!isVisible(level2))
				fadeIn(level2, 150 * cnt++);
			if (!isVisible(level3))
				fadeIn(level3, 150 * cnt++);
			if (!isVisible(level4))
				fadeIn(level4, 150 * cnt++);
			if (!isVisible(level5))
				fadeIn(level5, 150 * cnt++);
			if (!isVisible(level6))
				fadeIn(level6, 150 * cnt++);
			if (!isVisible(level7))
				fadeIn(level7, 150 * cnt++);
			locktype.setText(new LockFull(this).getShortType());
			description.setText(new LockFull(this).getDescription());
		} else if (level == 6) {
			int cnt = 0;
			if (isVisible(level7))
				fadeOut(level7, 150 * cnt++);
			if (!isVisible(level0))
				fadeIn(level0, 150 * cnt++);
			if (!isVisible(level1))
				fadeIn(level1, 150 * cnt++);
